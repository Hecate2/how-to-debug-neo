...
In practice, there is also `wallet address` and `script hash` (equivalent; mutually convertible) in Neo. These are the data generated by the public key. Wallet addresses (starting with `N` in Neo) make it easier for humans to input the target account, and script hashes (hexadecimal data) are hashes of verification scripts, helping smart contracts to transfer assets without knowing the public key of the receiver.
